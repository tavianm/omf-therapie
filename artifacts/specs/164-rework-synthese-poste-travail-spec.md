...
| R1 | `RendezVousView` — filtre « Demandes de RDV » | nouveau `FilterKey 'demandes'` : prédicat = appartenance à `getDemandItems` **composé avec la partition courante** (retour de test PR #167 : la bascule À venir/Historique reste visible et délimite le périmètre ; le filtre ne fusionne plus les deux partitions) + pilule dédiée dans `FILTERS` ; effet requête : `setFilter('demandes')`, `setPartition` selon la file — « À venir » si au moins une demande est à venir, sinon « Historique » directement (évolution test PR #167) —, `setQuery('')`, `setPage(1)` (garde nonce unique) | `getDemandItems`, requête union |
