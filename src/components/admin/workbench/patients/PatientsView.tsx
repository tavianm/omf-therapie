...
export function PatientsView({
  ...
  const allPatients = useMemo(
    () => aggregatePatients(appointments, nowMs),
    [appointments, nowMs],
  );
    ...
    <div>
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-sage-500 font-sans mt-1">
            ...
            {inactiveCount > 0
              ? ` · ${inactiveCount} archivé${inactiveCount > 1 ? 's' : ''}`
              : ''}
          </p>
        </div>
      ...
      <div className="mt-5 space-y-3">
        ...
        <div
          ...
          role="group"
          aria-label="Filtrer par initiale"
        >
          <button
            type="button"
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,5fr)_minmax(0,7fr)] lg:items-start">
                ...
                return (
                  <li key={patient.email}>
                    <button
                      type="button"
                      onClick={() =>
                        setSelectedEmail(isSelected ? null : patient.email)
                      }
                      ...
                      <span className="flex-1 min-w-0">
                        <span className="flex items-center gap-2">
                          ...
                          <span className="text-xs font-sans text-sage-500 shrink-0">
                            {patient.sessionCount} séance
                            {patient.sessionCount > 1 ? 's' : ''}
                          </span>
                        </span>
        ...
        <aside
          ...
          aria-label="Dossier du patient sélectionné"
        >
...
function Dossier({ patient, onClose, onPlan }: DossierProps) {
  ...
  return (
    <article
      ...
      aria-label={`Dossier de ${patient.name}`}
    >
      {/* Identité */}
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="inline-flex items-center gap-1.5 rounded-full bg-mint-100 px-2.5 py-0.5 text-xs font-medium font-sans text-mint-900">
              <span
                className={`w-1.5 h-1.5 rounded-full ${patient.isActive ? 'bg-mint-500' : 'bg-sage-400'}`}
                aria-hidden="true"
              />
          ...
          <p className="text-sm text-sage-500 font-sans mt-0.5">
            {getTypeLabel(patient.lastType)} · Première séance le{' '}
            {formatDateLong(patient.firstAppointmentAt)}
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="mt-4 rounded-xl bg-mint-50 p-4 space-y-2.5">
        {patient.phone && (
          <p className="flex items-center gap-2.5 text-sm font-sans text-sage-900">
            <svg
              className="w-4 h-4 text-sage-400 shrink-0"
              viewBox="0 0 20 20"
              fill="currentColor"
              aria-hidden="true"
            >
            ...
            <a
              ...
              className="hover:text-mint-700 transition-colors focus:outline-none focus:ring-2 focus:ring-mint-400 rounded"
            >
              {patient.phone}
            </a>
        ...
        <p className="flex items-center gap-2.5 text-sm font-sans text-sage-900">
          ...
          <a
            ...
            className="hover:text-mint-700 transition-colors break-all focus:outline-none focus:ring-2 focus:ring-mint-400 rounded"
          >
            {patient.email}
          </a>
      ...
      <div className="mt-3 grid grid-cols-2 gap-2.5">
        <div className="rounded-xl bg-mint-50 px-4 py-3">
          ...
          <p className="mt-1 font-serif text-lg font-semibold text-sage-900">
            ...
            <span className="font-sans text-xs font-normal text-sage-500">
              réalisée{patient.completedCount > 1 ? 's' : ''}
            </span>
          </p>
        </div>
        <div className="rounded-xl bg-mint-50 px-4 py-3">
          ...
          <p className="mt-1 font-serif text-lg font-semibold text-sage-900">
            {patient.pendingPaymentCents > 0 ? (
              <>
                ...
                <span className="font-sans text-xs font-normal text-amber-700">
                  en attente
                </span>
              </>
            ) : patient.paidCents > 0 ? (
      ...
      <div className="mt-3 rounded-xl border border-dashed border-sage-300 px-4 py-3 flex items-center justify-between gap-3">
        <p className="text-sm font-sans text-sage-500">
          Notes cliniques confidentielles
        </p>
      ...
      {reviewMessage && (
        <p
          role="status"
          aria-live="polite"
          className="mt-2 text-sm font-sans text-sage-700"
        >
          {reviewMessage}
        </p>
      )}
      ...
      <section className="mt-5" aria-label="Historique des séances">
        ...
        <ul className="space-y-2">
          {patient.history.map(appointment => {
            ...
            const isNext =
              patient.nextAppointment?.id === appointment.id &&
              isUpcoming(appointment.scheduled_at);
            return (
              <li
                key={appointment.id}
                className="flex items-center gap-3 rounded-xl border border-sage-200 bg-white px-3 py-2.5"
              >
                ...
                <span className="flex-1 min-w-0">
                  <span className="block text-sm font-medium font-sans text-sage-900 truncate">
                    {isNext ? 'Prochaine séance' : 'Séance'} ·{' '}
