...
export function RendezVousView({ appointments, focus, onRefresh }: RendezVousViewProps) {
  ...
  return (
    <div>
            ...
            return (
              <button
                ...
                className={`
                    ...
                    ${isActive ? 'bg-sage-900 text-white shadow-xs' : 'text-sage-600 hover:text-sage-900'}
                  `}
              >
                {label}
              </button>
      ...
      {/* ── Recherche + pastilles ─────────────────────────────────────────── */}
      <div className="mt-5 rounded-2xl border border-sage-200 bg-white p-3 space-y-3 shadow-xs">
        <div className="relative">
          ...
          {query && (
            <button
              ...
              className="absolute right-2.5 top-1/2 -translate-y-1/2 w-6 h-6 inline-flex items-center justify-center rounded-full text-sage-400 hover:text-sage-700 hover:bg-sage-100 focus:outline-hidden focus:ring-2 focus:ring-mint-400"
            >
              <svg
          ...
          {listFiltered.length > PAGE_SIZE && (
            ...
            >
              <button
                ...
                disabled={safePage <= 1}
                className="px-3 py-2 text-sm font-medium font-sans rounded-xl text-sage-600 hover:bg-mint-200/60 focus:outline-hidden focus:ring-2 focus:ring-mint-400 disabled:opacity-40 disabled:cursor-not-allowed min-h-[40px]"
              >
                ‹ Précédent
              </button>
