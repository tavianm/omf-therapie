...
async function parseErrorResponse(response: Response): Promise<string> {
  ...
  try {
    ...
    // A gateway or proxy may return HTML instead of the API JSON contract.
  }
...
export function TimeSlotManager() {
  ...
  const [deleteConfirmSlot, setDeleteConfirmSlot] =
    useState<ManualTimeSlot | null>(null);
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      if (editingSlot) {
        ...
        toast.success('Plage horaire mise à jour avec succès', {
          duration: 4000,
        });
      } else {
        // Create new slot
  ...
  return (
    <section className="space-y-4" aria-labelledby="slots-heading">
      ...
      <div className="flex items-center justify-between gap-3">
        <h2 id="slots-heading" className="font-serif text-xl text-sage-800">
        ...
        <ul className="space-y-3">
          {slots.map(slot => {
            ...
            return (
              ...
              >
                <div id={panelId} className="px-4 py-4">
      ...
      {deleteConfirmSlot && (
        <Modal
          ...
          onClose={() => {
            if (!isDeleting) setDeleteConfirmSlot(null);
          }}
        >
