          ...
          {faqs.map((faq, index) => (
            ...
            >
              <summary className="flex items-center justify-between p-4 cursor-pointer list-none bg-sage-50 hover:bg-sage-100 transition-colors">
                <dt className="font-medium text-sage-800 pr-4">{faq.question}</dt>
                <span className="shrink-0 text-sage-500 group-open:rotate-180 transition-transform">
                  ▾
                </span>
              </summary>
              <dd className="p-4 text-sage-600 text-sm leading-relaxed">
                {faq.answer}
              </dd>
            </details>
          ))}
